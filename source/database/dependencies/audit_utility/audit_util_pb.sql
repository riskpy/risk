...
BEGIN
  ...
  if p_update_cols is null then
    ...
    exception
     ...
     update audit_util_update_trig
     set    update_cols  = null
     where  table_name   = upper(p_table_name);
     l_update_cols := null;
  else
    begin
      insert into audit_util_update_trig (table_name,update_cols)
  ...
  if p_when_clause is null then
    ...
    begin
      insert into audit_util_update_trig (table_name, when_clause)
