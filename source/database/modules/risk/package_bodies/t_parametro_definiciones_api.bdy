CREATE OR REPLACE PACKAGE BODY T_PARAMETRO_DEFINICIONES_API IS
  /*
  This is the API for the table T_PARAMETRO_DEFINICIONES.
  - generator: OM_TAPIGEN
  - generator_version: 0.6.3
  - generator_action: COMPILE_API
  - generated_at: 2026-03-10 22:59:50
  - generated_by: JAVIER
  */

  g_bulk_limit     PLS_INTEGER := 1000;
  g_bulk_completed BOOLEAN     := FALSE;

  FUNCTION bulk_is_complete
  RETURN BOOLEAN
  IS
  BEGIN
    RETURN g_bulk_completed;
  END bulk_is_complete;

  PROCEDURE set_bulk_limit (
    p_bulk_limit IN PLS_INTEGER )
  IS
  BEGIN
    g_bulk_limit := p_bulk_limit;
  END set_bulk_limit;

  FUNCTION get_bulk_limit
  RETURN PLS_INTEGER
  IS
  BEGIN
    RETURN g_bulk_limit;
  END get_bulk_limit;

  FUNCTION row_exists (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN BOOLEAN
  IS
    v_return BOOLEAN := FALSE;
    v_dummy  PLS_INTEGER;
    CURSOR   cur_bool IS
      SELECT 1 FROM T_PARAMETRO_DEFINICIONES
      WHERE
        TABLA = p_tabla
        AND ID_PARAMETRO = p_id_parametro;
  BEGIN
    OPEN cur_bool;
    FETCH cur_bool INTO v_dummy;
    IF cur_bool%FOUND THEN
      v_return := TRUE;
    END IF;
    CLOSE cur_bool;
    RETURN v_return;
  END;

  FUNCTION row_exists_yn (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN VARCHAR2
  IS
  BEGIN
    RETURN
      CASE
        WHEN row_exists(
          p_tabla        => p_tabla,
          p_id_parametro => p_id_parametro )
        THEN 'Y'
        ELSE 'N'
      END;
  END;

  FUNCTION create_row (
    p_tabla             IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro      IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion       IN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE,
    p_orden             IN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE,
    p_nombre_referencia IN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE,
    p_tipo_dato         IN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE,
    p_observacion       IN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE,
    p_id_dominio        IN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE /*FK*/,
    p_tipo_filtro       IN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE,
    p_formato           IN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE,
    p_longitud_maxima   IN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE,
    p_obligatorio       IN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE,
    p_valor_defecto     IN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE,
    p_etiqueta          IN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE,
    p_valores_posibles  IN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE,
    p_encriptado        IN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE )
  RETURN T_PARAMETRO_DEFINICIONES%ROWTYPE
  IS
    v_return T_PARAMETRO_DEFINICIONES%ROWTYPE; 
  BEGIN
    INSERT INTO T_PARAMETRO_DEFINICIONES (
      TABLA /*PK*/,
      ID_PARAMETRO /*PK*/,
      DESCRIPCION,
      ORDEN,
      NOMBRE_REFERENCIA,
      TIPO_DATO,
      OBSERVACION,
      ID_DOMINIO /*FK*/,
      TIPO_FILTRO,
      FORMATO,
      LONGITUD_MAXIMA,
      OBLIGATORIO,
      VALOR_DEFECTO,
      ETIQUETA,
      VALORES_POSIBLES,
      ENCRIPTADO,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_tabla,
      p_id_parametro,
      p_descripcion,
      p_orden,
      p_nombre_referencia,
      p_tipo_dato,
      p_observacion,
      p_id_dominio,
      p_tipo_filtro,
      p_formato,
      p_longitud_maxima,
      p_obligatorio,
      p_valor_defecto,
      p_etiqueta,
      p_valores_posibles,
      p_encriptado,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp )
    RETURN 
      TABLA /*PK*/,
      ID_PARAMETRO /*PK*/,
      DESCRIPCION,
      ORDEN,
      NOMBRE_REFERENCIA,
      TIPO_DATO,
      OBSERVACION,
      ID_DOMINIO /*FK*/,
      TIPO_FILTRO,
      FORMATO,
      LONGITUD_MAXIMA,
      OBLIGATORIO,
      VALOR_DEFECTO,
      ETIQUETA,
      VALORES_POSIBLES,
      ENCRIPTADO,
      USUARIO_INSERCION,
      FECHA_INSERCION,
      USUARIO_MODIFICACION,
      FECHA_MODIFICACION
    INTO
      v_return; 
    RETURN v_return;
  END create_row;

  PROCEDURE create_row (
    p_tabla             IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro      IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion       IN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE,
    p_orden             IN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE,
    p_nombre_referencia IN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE,
    p_tipo_dato         IN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE,
    p_observacion       IN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE,
    p_id_dominio        IN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE /*FK*/,
    p_tipo_filtro       IN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE,
    p_formato           IN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE,
    p_longitud_maxima   IN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE,
    p_obligatorio       IN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE,
    p_valor_defecto     IN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE,
    p_etiqueta          IN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE,
    p_valores_posibles  IN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE,
    p_encriptado        IN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE )
  IS
  BEGIN
    INSERT INTO T_PARAMETRO_DEFINICIONES (
      TABLA /*PK*/,
      ID_PARAMETRO /*PK*/,
      DESCRIPCION,
      ORDEN,
      NOMBRE_REFERENCIA,
      TIPO_DATO,
      OBSERVACION,
      ID_DOMINIO /*FK*/,
      TIPO_FILTRO,
      FORMATO,
      LONGITUD_MAXIMA,
      OBLIGATORIO,
      VALOR_DEFECTO,
      ETIQUETA,
      VALORES_POSIBLES,
      ENCRIPTADO,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_tabla,
      p_id_parametro,
      p_descripcion,
      p_orden,
      p_nombre_referencia,
      p_tipo_dato,
      p_observacion,
      p_id_dominio,
      p_tipo_filtro,
      p_formato,
      p_longitud_maxima,
      p_obligatorio,
      p_valor_defecto,
      p_etiqueta,
      p_valores_posibles,
      p_encriptado,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp );
  END create_row;

  FUNCTION create_row (
    p_row IN T_PARAMETRO_DEFINICIONES%ROWTYPE )
  RETURN T_PARAMETRO_DEFINICIONES%ROWTYPE
  IS
  BEGIN
    RETURN create_row (
      p_tabla             => p_row.TABLA /*PK*/,
      p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
      p_descripcion       => p_row.DESCRIPCION,
      p_orden             => p_row.ORDEN,
      p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
      p_tipo_dato         => p_row.TIPO_DATO,
      p_observacion       => p_row.OBSERVACION,
      p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
      p_tipo_filtro       => p_row.TIPO_FILTRO,
      p_formato           => p_row.FORMATO,
      p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
      p_obligatorio       => p_row.OBLIGATORIO,
      p_valor_defecto     => p_row.VALOR_DEFECTO,
      p_etiqueta          => p_row.ETIQUETA,
      p_valores_posibles  => p_row.VALORES_POSIBLES,
      p_encriptado        => p_row.ENCRIPTADO );
  END create_row;

  PROCEDURE create_row (
    p_row IN T_PARAMETRO_DEFINICIONES%ROWTYPE )
  IS
  BEGIN
    create_row (
      p_tabla             => p_row.TABLA /*PK*/,
      p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
      p_descripcion       => p_row.DESCRIPCION,
      p_orden             => p_row.ORDEN,
      p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
      p_tipo_dato         => p_row.TIPO_DATO,
      p_observacion       => p_row.OBSERVACION,
      p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
      p_tipo_filtro       => p_row.TIPO_FILTRO,
      p_formato           => p_row.FORMATO,
      p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
      p_obligatorio       => p_row.OBLIGATORIO,
      p_valor_defecto     => p_row.VALOR_DEFECTO,
      p_etiqueta          => p_row.ETIQUETA,
      p_valores_posibles  => p_row.VALORES_POSIBLES,
      p_encriptado        => p_row.ENCRIPTADO );
  END create_row;

  FUNCTION create_rows (
    p_rows_tab IN t_rows_tab )
  RETURN t_rows_tab
  IS
    v_return t_rows_tab;
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
    INSERT INTO T_PARAMETRO_DEFINICIONES (
      TABLA /*PK*/,
      ID_PARAMETRO /*PK*/,
      DESCRIPCION,
      ORDEN,
      NOMBRE_REFERENCIA,
      TIPO_DATO,
      OBSERVACION,
      ID_DOMINIO /*FK*/,
      TIPO_FILTRO,
      FORMATO,
      LONGITUD_MAXIMA,
      OBLIGATORIO,
      VALOR_DEFECTO,
      ETIQUETA,
      VALORES_POSIBLES,
      ENCRIPTADO,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_rows_tab(i).TABLA,
      p_rows_tab(i).ID_PARAMETRO,
      p_rows_tab(i).DESCRIPCION,
      p_rows_tab(i).ORDEN,
      p_rows_tab(i).NOMBRE_REFERENCIA,
      p_rows_tab(i).TIPO_DATO,
      p_rows_tab(i).OBSERVACION,
      p_rows_tab(i).ID_DOMINIO,
      p_rows_tab(i).TIPO_FILTRO,
      p_rows_tab(i).FORMATO,
      p_rows_tab(i).LONGITUD_MAXIMA,
      p_rows_tab(i).OBLIGATORIO,
      p_rows_tab(i).VALOR_DEFECTO,
      p_rows_tab(i).ETIQUETA,
      p_rows_tab(i).VALORES_POSIBLES,
      p_rows_tab(i).ENCRIPTADO,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp )
    RETURN 
      TABLA /*PK*/,
      ID_PARAMETRO /*PK*/,
      DESCRIPCION,
      ORDEN,
      NOMBRE_REFERENCIA,
      TIPO_DATO,
      OBSERVACION,
      ID_DOMINIO /*FK*/,
      TIPO_FILTRO,
      FORMATO,
      LONGITUD_MAXIMA,
      OBLIGATORIO,
      VALOR_DEFECTO,
      ETIQUETA,
      VALORES_POSIBLES,
      ENCRIPTADO,
      USUARIO_INSERCION,
      FECHA_INSERCION,
      USUARIO_MODIFICACION,
      FECHA_MODIFICACION
    BULK COLLECT INTO v_return;
    RETURN v_return;
  END create_rows;

  PROCEDURE create_rows (
    p_rows_tab IN t_rows_tab )
  IS
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
    INSERT INTO T_PARAMETRO_DEFINICIONES (
      TABLA /*PK*/,
      ID_PARAMETRO /*PK*/,
      DESCRIPCION,
      ORDEN,
      NOMBRE_REFERENCIA,
      TIPO_DATO,
      OBSERVACION,
      ID_DOMINIO /*FK*/,
      TIPO_FILTRO,
      FORMATO,
      LONGITUD_MAXIMA,
      OBLIGATORIO,
      VALOR_DEFECTO,
      ETIQUETA,
      VALORES_POSIBLES,
      ENCRIPTADO,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_rows_tab(i).TABLA,
      p_rows_tab(i).ID_PARAMETRO,
      p_rows_tab(i).DESCRIPCION,
      p_rows_tab(i).ORDEN,
      p_rows_tab(i).NOMBRE_REFERENCIA,
      p_rows_tab(i).TIPO_DATO,
      p_rows_tab(i).OBSERVACION,
      p_rows_tab(i).ID_DOMINIO,
      p_rows_tab(i).TIPO_FILTRO,
      p_rows_tab(i).FORMATO,
      p_rows_tab(i).LONGITUD_MAXIMA,
      p_rows_tab(i).OBLIGATORIO,
      p_rows_tab(i).VALOR_DEFECTO,
      p_rows_tab(i).ETIQUETA,
      p_rows_tab(i).VALORES_POSIBLES,
      p_rows_tab(i).ENCRIPTADO,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp );
  END create_rows;

  FUNCTION read_row (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES%ROWTYPE
  IS
    v_row T_PARAMETRO_DEFINICIONES%ROWTYPE;
    CURSOR cur_row IS
      SELECT * FROM T_PARAMETRO_DEFINICIONES
      WHERE
        TABLA = p_tabla
        AND ID_PARAMETRO = p_id_parametro;
  BEGIN
    OPEN cur_row;
    FETCH cur_row INTO v_row;
    CLOSE cur_row;
    RETURN v_row;
  END read_row;

  PROCEDURE read_row (
    p_tabla                IN            T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro         IN            T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion             OUT NOCOPY T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE,
    p_orden                   OUT NOCOPY T_PARAMETRO_DEFINICIONES.ORDEN%TYPE,
    p_nombre_referencia       OUT NOCOPY T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE,
    p_tipo_dato               OUT NOCOPY T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE,
    p_observacion             OUT NOCOPY T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE,
    p_id_dominio              OUT NOCOPY T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE /*FK*/,
    p_tipo_filtro             OUT NOCOPY T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE,
    p_formato                 OUT NOCOPY T_PARAMETRO_DEFINICIONES.FORMATO%TYPE,
    p_longitud_maxima         OUT NOCOPY T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE,
    p_obligatorio             OUT NOCOPY T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE,
    p_valor_defecto           OUT NOCOPY T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE,
    p_etiqueta                OUT NOCOPY T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE,
    p_valores_posibles        OUT NOCOPY T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE,
    p_encriptado              OUT NOCOPY T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE,
    p_usuario_insercion       OUT NOCOPY T_PARAMETRO_DEFINICIONES.USUARIO_INSERCION%TYPE,
    p_fecha_insercion         OUT NOCOPY T_PARAMETRO_DEFINICIONES.FECHA_INSERCION%TYPE,
    p_usuario_modificacion    OUT NOCOPY T_PARAMETRO_DEFINICIONES.USUARIO_MODIFICACION%TYPE,
    p_fecha_modificacion      OUT NOCOPY T_PARAMETRO_DEFINICIONES.FECHA_MODIFICACION%TYPE )
  IS
    v_row T_PARAMETRO_DEFINICIONES%ROWTYPE;
  BEGIN
    v_row := read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro );
    p_descripcion          := v_row.DESCRIPCION; 
    p_orden                := v_row.ORDEN; 
    p_nombre_referencia    := v_row.NOMBRE_REFERENCIA; 
    p_tipo_dato            := v_row.TIPO_DATO; 
    p_observacion          := v_row.OBSERVACION; 
    p_id_dominio           := v_row.ID_DOMINIO; 
    p_tipo_filtro          := v_row.TIPO_FILTRO; 
    p_formato              := v_row.FORMATO; 
    p_longitud_maxima      := v_row.LONGITUD_MAXIMA; 
    p_obligatorio          := v_row.OBLIGATORIO; 
    p_valor_defecto        := v_row.VALOR_DEFECTO; 
    p_etiqueta             := v_row.ETIQUETA; 
    p_valores_posibles     := v_row.VALORES_POSIBLES; 
    p_encriptado           := v_row.ENCRIPTADO; 
    p_usuario_insercion    := v_row.USUARIO_INSERCION; 
    p_fecha_insercion      := v_row.FECHA_INSERCION; 
    p_usuario_modificacion := v_row.USUARIO_MODIFICACION; 
    p_fecha_modificacion   := v_row.FECHA_MODIFICACION; 
  END read_row;

  FUNCTION read_rows (
    p_ref_cursor IN t_strong_ref_cursor )
  RETURN t_rows_tab
  IS
    v_return t_rows_tab;
  BEGIN
    IF (p_ref_cursor%ISOPEN) THEN
      g_bulk_completed := FALSE;
      FETCH p_ref_cursor BULK COLLECT INTO v_return LIMIT g_bulk_limit;
      IF (v_return.COUNT < g_bulk_limit) THEN
        g_bulk_completed := TRUE;
      END IF;
    END IF;
    RETURN v_return;
  END read_rows;

  FUNCTION update_row (
    p_tabla             IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro      IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion       IN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE,
    p_orden             IN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE,
    p_nombre_referencia IN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE,
    p_tipo_dato         IN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE,
    p_observacion       IN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE,
    p_id_dominio        IN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE /*FK*/,
    p_tipo_filtro       IN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE,
    p_formato           IN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE,
    p_longitud_maxima   IN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE,
    p_obligatorio       IN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE,
    p_valor_defecto     IN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE,
    p_etiqueta          IN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE,
    p_valores_posibles  IN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE,
    p_encriptado        IN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE )
  RETURN T_PARAMETRO_DEFINICIONES%ROWTYPE
  IS
    v_return T_PARAMETRO_DEFINICIONES%ROWTYPE; 
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      DESCRIPCION          = p_descripcion,
      ORDEN                = p_orden,
      NOMBRE_REFERENCIA    = p_nombre_referencia,
      TIPO_DATO            = p_tipo_dato,
      OBSERVACION          = p_observacion,
      ID_DOMINIO           = p_id_dominio /*FK*/,
      TIPO_FILTRO          = p_tipo_filtro,
      FORMATO              = p_formato,
      LONGITUD_MAXIMA      = p_longitud_maxima,
      OBLIGATORIO          = p_obligatorio,
      VALOR_DEFECTO        = p_valor_defecto,
      ETIQUETA             = p_etiqueta,
      VALORES_POSIBLES     = p_valores_posibles,
      ENCRIPTADO           = p_encriptado,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro
    RETURN 
      TABLA /*PK*/,
      ID_PARAMETRO /*PK*/,
      DESCRIPCION,
      ORDEN,
      NOMBRE_REFERENCIA,
      TIPO_DATO,
      OBSERVACION,
      ID_DOMINIO /*FK*/,
      TIPO_FILTRO,
      FORMATO,
      LONGITUD_MAXIMA,
      OBLIGATORIO,
      VALOR_DEFECTO,
      ETIQUETA,
      VALORES_POSIBLES,
      ENCRIPTADO,
      USUARIO_INSERCION,
      FECHA_INSERCION,
      USUARIO_MODIFICACION,
      FECHA_MODIFICACION
    INTO
      v_return; 
    RETURN v_return;
  END update_row;

  PROCEDURE update_row (
    p_tabla             IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro      IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion       IN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE,
    p_orden             IN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE,
    p_nombre_referencia IN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE,
    p_tipo_dato         IN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE,
    p_observacion       IN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE,
    p_id_dominio        IN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE /*FK*/,
    p_tipo_filtro       IN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE,
    p_formato           IN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE,
    p_longitud_maxima   IN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE,
    p_obligatorio       IN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE,
    p_valor_defecto     IN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE,
    p_etiqueta          IN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE,
    p_valores_posibles  IN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE,
    p_encriptado        IN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      DESCRIPCION          = p_descripcion,
      ORDEN                = p_orden,
      NOMBRE_REFERENCIA    = p_nombre_referencia,
      TIPO_DATO            = p_tipo_dato,
      OBSERVACION          = p_observacion,
      ID_DOMINIO           = p_id_dominio /*FK*/,
      TIPO_FILTRO          = p_tipo_filtro,
      FORMATO              = p_formato,
      LONGITUD_MAXIMA      = p_longitud_maxima,
      OBLIGATORIO          = p_obligatorio,
      VALOR_DEFECTO        = p_valor_defecto,
      ETIQUETA             = p_etiqueta,
      VALORES_POSIBLES     = p_valores_posibles,
      ENCRIPTADO           = p_encriptado,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END update_row;

  FUNCTION update_row (
    p_row IN T_PARAMETRO_DEFINICIONES%ROWTYPE )
  RETURN T_PARAMETRO_DEFINICIONES%ROWTYPE
  IS
  BEGIN
    RETURN update_row (
      p_tabla             => p_row.TABLA /*PK*/,
      p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
      p_descripcion       => p_row.DESCRIPCION,
      p_orden             => p_row.ORDEN,
      p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
      p_tipo_dato         => p_row.TIPO_DATO,
      p_observacion       => p_row.OBSERVACION,
      p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
      p_tipo_filtro       => p_row.TIPO_FILTRO,
      p_formato           => p_row.FORMATO,
      p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
      p_obligatorio       => p_row.OBLIGATORIO,
      p_valor_defecto     => p_row.VALOR_DEFECTO,
      p_etiqueta          => p_row.ETIQUETA,
      p_valores_posibles  => p_row.VALORES_POSIBLES,
      p_encriptado        => p_row.ENCRIPTADO );
  END update_row;

  PROCEDURE update_row (
    p_row IN T_PARAMETRO_DEFINICIONES%ROWTYPE )
  IS
  BEGIN
    update_row (
      p_tabla             => p_row.TABLA /*PK*/,
      p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
      p_descripcion       => p_row.DESCRIPCION,
      p_orden             => p_row.ORDEN,
      p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
      p_tipo_dato         => p_row.TIPO_DATO,
      p_observacion       => p_row.OBSERVACION,
      p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
      p_tipo_filtro       => p_row.TIPO_FILTRO,
      p_formato           => p_row.FORMATO,
      p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
      p_obligatorio       => p_row.OBLIGATORIO,
      p_valor_defecto     => p_row.VALOR_DEFECTO,
      p_etiqueta          => p_row.ETIQUETA,
      p_valores_posibles  => p_row.VALORES_POSIBLES,
      p_encriptado        => p_row.ENCRIPTADO );
  END update_row;

  PROCEDURE update_rows (
    p_rows_tab IN t_rows_tab )
  IS
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
      UPDATE
        T_PARAMETRO_DEFINICIONES
      SET
        DESCRIPCION          = p_rows_tab(i).DESCRIPCION,
        ORDEN                = p_rows_tab(i).ORDEN,
        NOMBRE_REFERENCIA    = p_rows_tab(i).NOMBRE_REFERENCIA,
        TIPO_DATO            = p_rows_tab(i).TIPO_DATO,
        OBSERVACION          = p_rows_tab(i).OBSERVACION,
        ID_DOMINIO           = p_rows_tab(i).ID_DOMINIO /*FK*/,
        TIPO_FILTRO          = p_rows_tab(i).TIPO_FILTRO,
        FORMATO              = p_rows_tab(i).FORMATO,
        LONGITUD_MAXIMA      = p_rows_tab(i).LONGITUD_MAXIMA,
        OBLIGATORIO          = p_rows_tab(i).OBLIGATORIO,
        VALOR_DEFECTO        = p_rows_tab(i).VALOR_DEFECTO,
        ETIQUETA             = p_rows_tab(i).ETIQUETA,
        VALORES_POSIBLES     = p_rows_tab(i).VALORES_POSIBLES,
        ENCRIPTADO           = p_rows_tab(i).ENCRIPTADO,
        USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
        FECHA_MODIFICACION   = systimestamp
      WHERE
        TABLA = p_rows_tab(i).TABLA
        AND ID_PARAMETRO = p_rows_tab(i).ID_PARAMETRO;
  END update_rows;

  PROCEDURE delete_row (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  IS
  BEGIN
    DELETE FROM T_PARAMETRO_DEFINICIONES
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END delete_row;

  PROCEDURE delete_rows (
    p_rows_tab IN t_rows_tab )
  IS
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
      DELETE FROM T_PARAMETRO_DEFINICIONES
       WHERE TABLA = p_rows_tab(i).TABLA
        AND ID_PARAMETRO = p_rows_tab(i).ID_PARAMETRO;
  END delete_rows;

  FUNCTION create_or_update_row (
    p_tabla             IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro      IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion       IN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE,
    p_orden             IN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE,
    p_nombre_referencia IN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE,
    p_tipo_dato         IN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE,
    p_observacion       IN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE,
    p_id_dominio        IN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE /*FK*/,
    p_tipo_filtro       IN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE,
    p_formato           IN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE,
    p_longitud_maxima   IN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE,
    p_obligatorio       IN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE,
    p_valor_defecto     IN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE,
    p_etiqueta          IN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE,
    p_valores_posibles  IN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE,
    p_encriptado        IN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE )
  RETURN T_PARAMETRO_DEFINICIONES%ROWTYPE
  IS
  BEGIN
    IF row_exists(
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro
    )
    THEN
      RETURN update_row (
        p_tabla             => p_tabla /*PK*/,
        p_id_parametro      => p_id_parametro /*PK*/,
        p_descripcion       => p_descripcion,
        p_orden             => p_orden,
        p_nombre_referencia => p_nombre_referencia,
        p_tipo_dato         => p_tipo_dato,
        p_observacion       => p_observacion,
        p_id_dominio        => p_id_dominio /*FK*/,
        p_tipo_filtro       => p_tipo_filtro,
        p_formato           => p_formato,
        p_longitud_maxima   => p_longitud_maxima,
        p_obligatorio       => p_obligatorio,
        p_valor_defecto     => p_valor_defecto,
        p_etiqueta          => p_etiqueta,
        p_valores_posibles  => p_valores_posibles,
        p_encriptado        => p_encriptado );
    ELSE
      RETURN create_row (
        p_tabla             => p_tabla /*PK*/,
        p_id_parametro      => p_id_parametro /*PK*/,
        p_descripcion       => p_descripcion,
        p_orden             => p_orden,
        p_nombre_referencia => p_nombre_referencia,
        p_tipo_dato         => p_tipo_dato,
        p_observacion       => p_observacion,
        p_id_dominio        => p_id_dominio /*FK*/,
        p_tipo_filtro       => p_tipo_filtro,
        p_formato           => p_formato,
        p_longitud_maxima   => p_longitud_maxima,
        p_obligatorio       => p_obligatorio,
        p_valor_defecto     => p_valor_defecto,
        p_etiqueta          => p_etiqueta,
        p_valores_posibles  => p_valores_posibles,
        p_encriptado        => p_encriptado );
    END IF;
  END create_or_update_row;

  PROCEDURE create_or_update_row (
    p_tabla             IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro      IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion       IN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE,
    p_orden             IN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE,
    p_nombre_referencia IN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE,
    p_tipo_dato         IN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE,
    p_observacion       IN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE,
    p_id_dominio        IN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE /*FK*/,
    p_tipo_filtro       IN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE,
    p_formato           IN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE,
    p_longitud_maxima   IN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE,
    p_obligatorio       IN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE,
    p_valor_defecto     IN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE,
    p_etiqueta          IN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE,
    p_valores_posibles  IN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE,
    p_encriptado        IN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE )
  IS
  BEGIN
    IF row_exists(
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro
    )
    THEN
      update_row (
        p_tabla             => p_tabla /*PK*/,
        p_id_parametro      => p_id_parametro /*PK*/,
        p_descripcion       => p_descripcion,
        p_orden             => p_orden,
        p_nombre_referencia => p_nombre_referencia,
        p_tipo_dato         => p_tipo_dato,
        p_observacion       => p_observacion,
        p_id_dominio        => p_id_dominio /*FK*/,
        p_tipo_filtro       => p_tipo_filtro,
        p_formato           => p_formato,
        p_longitud_maxima   => p_longitud_maxima,
        p_obligatorio       => p_obligatorio,
        p_valor_defecto     => p_valor_defecto,
        p_etiqueta          => p_etiqueta,
        p_valores_posibles  => p_valores_posibles,
        p_encriptado        => p_encriptado );
    ELSE
      create_row (
        p_tabla             => p_tabla /*PK*/,
        p_id_parametro      => p_id_parametro /*PK*/,
        p_descripcion       => p_descripcion,
        p_orden             => p_orden,
        p_nombre_referencia => p_nombre_referencia,
        p_tipo_dato         => p_tipo_dato,
        p_observacion       => p_observacion,
        p_id_dominio        => p_id_dominio /*FK*/,
        p_tipo_filtro       => p_tipo_filtro,
        p_formato           => p_formato,
        p_longitud_maxima   => p_longitud_maxima,
        p_obligatorio       => p_obligatorio,
        p_valor_defecto     => p_valor_defecto,
        p_etiqueta          => p_etiqueta,
        p_valores_posibles  => p_valores_posibles,
        p_encriptado        => p_encriptado );
    END IF;
  END create_or_update_row;

  FUNCTION create_or_update_row (
    p_row IN T_PARAMETRO_DEFINICIONES%ROWTYPE )
  RETURN T_PARAMETRO_DEFINICIONES%ROWTYPE
  IS
  BEGIN
    IF row_exists(
      p_tabla        => p_row.TABLA,
      p_id_parametro => p_row.ID_PARAMETRO
    )
    THEN
      RETURN update_row (
        p_tabla             => p_row.TABLA /*PK*/,
        p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
        p_descripcion       => p_row.DESCRIPCION,
        p_orden             => p_row.ORDEN,
        p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
        p_tipo_dato         => p_row.TIPO_DATO,
        p_observacion       => p_row.OBSERVACION,
        p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
        p_tipo_filtro       => p_row.TIPO_FILTRO,
        p_formato           => p_row.FORMATO,
        p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
        p_obligatorio       => p_row.OBLIGATORIO,
        p_valor_defecto     => p_row.VALOR_DEFECTO,
        p_etiqueta          => p_row.ETIQUETA,
        p_valores_posibles  => p_row.VALORES_POSIBLES,
        p_encriptado        => p_row.ENCRIPTADO );
    ELSE
      RETURN create_row (
        p_tabla             => p_row.TABLA /*PK*/,
        p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
        p_descripcion       => p_row.DESCRIPCION,
        p_orden             => p_row.ORDEN,
        p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
        p_tipo_dato         => p_row.TIPO_DATO,
        p_observacion       => p_row.OBSERVACION,
        p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
        p_tipo_filtro       => p_row.TIPO_FILTRO,
        p_formato           => p_row.FORMATO,
        p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
        p_obligatorio       => p_row.OBLIGATORIO,
        p_valor_defecto     => p_row.VALOR_DEFECTO,
        p_etiqueta          => p_row.ETIQUETA,
        p_valores_posibles  => p_row.VALORES_POSIBLES,
        p_encriptado        => p_row.ENCRIPTADO );
    END IF;
  END create_or_update_row;

  PROCEDURE create_or_update_row (
    p_row IN T_PARAMETRO_DEFINICIONES%ROWTYPE )
  IS
  BEGIN
    IF row_exists(
      p_tabla        => p_row.TABLA,
      p_id_parametro => p_row.ID_PARAMETRO
    )
    THEN
      update_row (
        p_tabla             => p_row.TABLA /*PK*/,
        p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
        p_descripcion       => p_row.DESCRIPCION,
        p_orden             => p_row.ORDEN,
        p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
        p_tipo_dato         => p_row.TIPO_DATO,
        p_observacion       => p_row.OBSERVACION,
        p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
        p_tipo_filtro       => p_row.TIPO_FILTRO,
        p_formato           => p_row.FORMATO,
        p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
        p_obligatorio       => p_row.OBLIGATORIO,
        p_valor_defecto     => p_row.VALOR_DEFECTO,
        p_etiqueta          => p_row.ETIQUETA,
        p_valores_posibles  => p_row.VALORES_POSIBLES,
        p_encriptado        => p_row.ENCRIPTADO );
    ELSE
      create_row (
        p_tabla             => p_row.TABLA /*PK*/,
        p_id_parametro      => p_row.ID_PARAMETRO /*PK*/,
        p_descripcion       => p_row.DESCRIPCION,
        p_orden             => p_row.ORDEN,
        p_nombre_referencia => p_row.NOMBRE_REFERENCIA,
        p_tipo_dato         => p_row.TIPO_DATO,
        p_observacion       => p_row.OBSERVACION,
        p_id_dominio        => p_row.ID_DOMINIO /*FK*/,
        p_tipo_filtro       => p_row.TIPO_FILTRO,
        p_formato           => p_row.FORMATO,
        p_longitud_maxima   => p_row.LONGITUD_MAXIMA,
        p_obligatorio       => p_row.OBLIGATORIO,
        p_valor_defecto     => p_row.VALOR_DEFECTO,
        p_etiqueta          => p_row.ETIQUETA,
        p_valores_posibles  => p_row.VALORES_POSIBLES,
        p_encriptado        => p_row.ENCRIPTADO );
    END IF;
  END create_or_update_row;

  FUNCTION get_descripcion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).DESCRIPCION;
  END get_descripcion;

  FUNCTION get_orden (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).ORDEN;
  END get_orden;

  FUNCTION get_nombre_referencia (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).NOMBRE_REFERENCIA;
  END get_nombre_referencia;

  FUNCTION get_tipo_dato (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).TIPO_DATO;
  END get_tipo_dato;

  FUNCTION get_observacion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).OBSERVACION;
  END get_observacion;

  FUNCTION get_id_dominio (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).ID_DOMINIO;
  END get_id_dominio;

  FUNCTION get_tipo_filtro (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).TIPO_FILTRO;
  END get_tipo_filtro;

  FUNCTION get_formato (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).FORMATO;
  END get_formato;

  FUNCTION get_longitud_maxima (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).LONGITUD_MAXIMA;
  END get_longitud_maxima;

  FUNCTION get_obligatorio (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).OBLIGATORIO;
  END get_obligatorio;

  FUNCTION get_valor_defecto (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).VALOR_DEFECTO;
  END get_valor_defecto;

  FUNCTION get_etiqueta (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).ETIQUETA;
  END get_etiqueta;

  FUNCTION get_valores_posibles (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).VALORES_POSIBLES;
  END get_valores_posibles;

  FUNCTION get_encriptado (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).ENCRIPTADO;
  END get_encriptado;

  FUNCTION get_usuario_insercion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.USUARIO_INSERCION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).USUARIO_INSERCION;
  END get_usuario_insercion;

  FUNCTION get_fecha_insercion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.FECHA_INSERCION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).FECHA_INSERCION;
  END get_fecha_insercion;

  FUNCTION get_usuario_modificacion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.USUARIO_MODIFICACION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).USUARIO_MODIFICACION;
  END get_usuario_modificacion;

  FUNCTION get_fecha_modificacion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/ )
  RETURN T_PARAMETRO_DEFINICIONES.FECHA_MODIFICACION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_tabla        => p_tabla,
      p_id_parametro => p_id_parametro ).FECHA_MODIFICACION;
  END get_fecha_modificacion;

  PROCEDURE set_descripcion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_descripcion  IN T_PARAMETRO_DEFINICIONES.DESCRIPCION%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      DESCRIPCION          = p_descripcion,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_descripcion;

  PROCEDURE set_orden (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_orden        IN T_PARAMETRO_DEFINICIONES.ORDEN%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      ORDEN                = p_orden,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_orden;

  PROCEDURE set_nombre_referencia (
    p_tabla             IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro      IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_nombre_referencia IN T_PARAMETRO_DEFINICIONES.NOMBRE_REFERENCIA%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      NOMBRE_REFERENCIA    = p_nombre_referencia,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_nombre_referencia;

  PROCEDURE set_tipo_dato (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_tipo_dato    IN T_PARAMETRO_DEFINICIONES.TIPO_DATO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      TIPO_DATO            = p_tipo_dato,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_tipo_dato;

  PROCEDURE set_observacion (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_observacion  IN T_PARAMETRO_DEFINICIONES.OBSERVACION%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      OBSERVACION          = p_observacion,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_observacion;

  PROCEDURE set_id_dominio (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_id_dominio   IN T_PARAMETRO_DEFINICIONES.ID_DOMINIO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      ID_DOMINIO           = p_id_dominio /*FK*/,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_id_dominio;

  PROCEDURE set_tipo_filtro (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_tipo_filtro  IN T_PARAMETRO_DEFINICIONES.TIPO_FILTRO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      TIPO_FILTRO          = p_tipo_filtro,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_tipo_filtro;

  PROCEDURE set_formato (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_formato      IN T_PARAMETRO_DEFINICIONES.FORMATO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      FORMATO              = p_formato,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_formato;

  PROCEDURE set_longitud_maxima (
    p_tabla           IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro    IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_longitud_maxima IN T_PARAMETRO_DEFINICIONES.LONGITUD_MAXIMA%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      LONGITUD_MAXIMA      = p_longitud_maxima,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_longitud_maxima;

  PROCEDURE set_obligatorio (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_obligatorio  IN T_PARAMETRO_DEFINICIONES.OBLIGATORIO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      OBLIGATORIO          = p_obligatorio,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_obligatorio;

  PROCEDURE set_valor_defecto (
    p_tabla         IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro  IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_valor_defecto IN T_PARAMETRO_DEFINICIONES.VALOR_DEFECTO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      VALOR_DEFECTO        = p_valor_defecto,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_valor_defecto;

  PROCEDURE set_etiqueta (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_etiqueta     IN T_PARAMETRO_DEFINICIONES.ETIQUETA%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      ETIQUETA             = p_etiqueta,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_etiqueta;

  PROCEDURE set_valores_posibles (
    p_tabla            IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro     IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_valores_posibles IN T_PARAMETRO_DEFINICIONES.VALORES_POSIBLES%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      VALORES_POSIBLES     = p_valores_posibles,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_valores_posibles;

  PROCEDURE set_encriptado (
    p_tabla        IN T_PARAMETRO_DEFINICIONES.TABLA%TYPE /*PK*/,
    p_id_parametro IN T_PARAMETRO_DEFINICIONES.ID_PARAMETRO%TYPE /*PK*/,
    p_encriptado   IN T_PARAMETRO_DEFINICIONES.ENCRIPTADO%TYPE )
  IS
  BEGIN
    UPDATE
      T_PARAMETRO_DEFINICIONES
    SET
      ENCRIPTADO           = p_encriptado,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      TABLA = p_tabla
      AND ID_PARAMETRO = p_id_parametro;
  END set_encriptado;

END T_PARAMETRO_DEFINICIONES_API;
/

