CREATE OR REPLACE PACKAGE BODY T_REPORTES_API IS
  /*
  This is the API for the table T_REPORTES.
  - generator: OM_TAPIGEN
  - generator_version: 0.6.3
  - generator_action: COMPILE_API
  - generated_at: 2026-03-15 15:13:58
  - generated_by: JAVIER
  */

  g_bulk_limit     PLS_INTEGER := 1000;
  g_bulk_completed BOOLEAN     := FALSE;

  FUNCTION bulk_is_complete
  RETURN BOOLEAN
  IS
  BEGIN
    RETURN g_bulk_completed;
  END bulk_is_complete;

  PROCEDURE set_bulk_limit (
    p_bulk_limit IN PLS_INTEGER )
  IS
  BEGIN
    g_bulk_limit := p_bulk_limit;
  END set_bulk_limit;

  FUNCTION get_bulk_limit
  RETURN PLS_INTEGER
  IS
  BEGIN
    RETURN g_bulk_limit;
  END get_bulk_limit;

  FUNCTION row_exists (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN BOOLEAN
  IS
    v_return BOOLEAN := FALSE;
    v_dummy  PLS_INTEGER;
    CURSOR   cur_bool IS
      SELECT 1 FROM T_REPORTES
      WHERE
        ID_REPORTE = p_id_reporte;
  BEGIN
    OPEN cur_bool;
    FETCH cur_bool INTO v_dummy;
    IF cur_bool%FOUND THEN
      v_return := TRUE;
    END IF;
    CLOSE cur_bool;
    RETURN v_return;
  END;

  FUNCTION row_exists_yn (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN VARCHAR2
  IS
  BEGIN
    RETURN
      CASE
        WHEN row_exists(
          p_id_reporte => p_id_reporte )
        THEN 'Y'
        ELSE 'N'
      END;
  END;

  FUNCTION create_row (
    p_id_reporte             IN T_REPORTES.ID_REPORTE%TYPE               DEFAULT NULL /*PK*/ /*FK*/,
    p_tipo                   IN T_REPORTES.TIPO%TYPE                     ,
    p_cantidad_ejecuciones   IN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE     DEFAULT NULL,
    p_fecha_ultima_ejecucion IN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE   DEFAULT NULL,
    p_consulta_sql           IN T_REPORTES.CONSULTA_SQL%TYPE             DEFAULT NULL,
    p_sql_ultima_ejecucion   IN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE     DEFAULT NULL )
  RETURN T_REPORTES.ID_REPORTE%TYPE
  IS
    v_return T_REPORTES.ID_REPORTE%TYPE; 
  BEGIN
    INSERT INTO T_REPORTES (
      ID_REPORTE /*PK*/ /*FK*/,
      TIPO,
      CANTIDAD_EJECUCIONES,
      FECHA_ULTIMA_EJECUCION,
      CONSULTA_SQL,
      SQL_ULTIMA_EJECUCION,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_id_reporte,
      p_tipo,
      p_cantidad_ejecuciones,
      p_fecha_ultima_ejecucion,
      p_consulta_sql,
      p_sql_ultima_ejecucion,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp )
    RETURN 
      ID_REPORTE
    INTO
      v_return;
    RETURN v_return;
  END create_row;

  PROCEDURE create_row (
    p_id_reporte             IN T_REPORTES.ID_REPORTE%TYPE               DEFAULT NULL /*PK*/ /*FK*/,
    p_tipo                   IN T_REPORTES.TIPO%TYPE                     ,
    p_cantidad_ejecuciones   IN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE     DEFAULT NULL,
    p_fecha_ultima_ejecucion IN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE   DEFAULT NULL,
    p_consulta_sql           IN T_REPORTES.CONSULTA_SQL%TYPE             DEFAULT NULL,
    p_sql_ultima_ejecucion   IN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE     DEFAULT NULL )
  IS
  BEGIN
    INSERT INTO T_REPORTES (
      ID_REPORTE /*PK*/ /*FK*/,
      TIPO,
      CANTIDAD_EJECUCIONES,
      FECHA_ULTIMA_EJECUCION,
      CONSULTA_SQL,
      SQL_ULTIMA_EJECUCION,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_id_reporte,
      p_tipo,
      p_cantidad_ejecuciones,
      p_fecha_ultima_ejecucion,
      p_consulta_sql,
      p_sql_ultima_ejecucion,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp );
  END create_row;

  FUNCTION create_row (
    p_row IN T_REPORTES%ROWTYPE )
  RETURN T_REPORTES.ID_REPORTE%TYPE
  IS
  BEGIN
    RETURN create_row (
      p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
      p_tipo                   => p_row.TIPO,
      p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
      p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
      p_consulta_sql           => p_row.CONSULTA_SQL,
      p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
  END create_row;

  PROCEDURE create_row (
    p_row IN T_REPORTES%ROWTYPE )
  IS
  BEGIN
    create_row (
      p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
      p_tipo                   => p_row.TIPO,
      p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
      p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
      p_consulta_sql           => p_row.CONSULTA_SQL,
      p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
  END create_row;

  FUNCTION create_rows (
    p_rows_tab IN t_rows_tab )
  RETURN t_rows_tab
  IS
    v_return t_rows_tab;
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
    INSERT INTO T_REPORTES (
      ID_REPORTE /*PK*/ /*FK*/,
      TIPO,
      CANTIDAD_EJECUCIONES,
      FECHA_ULTIMA_EJECUCION,
      CONSULTA_SQL,
      SQL_ULTIMA_EJECUCION,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_rows_tab(i).ID_REPORTE,
      p_rows_tab(i).TIPO,
      p_rows_tab(i).CANTIDAD_EJECUCIONES,
      p_rows_tab(i).FECHA_ULTIMA_EJECUCION,
      p_rows_tab(i).CONSULTA_SQL,
      p_rows_tab(i).SQL_ULTIMA_EJECUCION,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp )
    RETURN 
      ID_REPORTE /*PK*/ /*FK*/,
      TIPO,
      CANTIDAD_EJECUCIONES,
      FECHA_ULTIMA_EJECUCION,
      CONSULTA_SQL,
      SQL_ULTIMA_EJECUCION,
      USUARIO_INSERCION,
      FECHA_INSERCION,
      USUARIO_MODIFICACION,
      FECHA_MODIFICACION
    BULK COLLECT INTO v_return;
    RETURN v_return;
  END create_rows;

  PROCEDURE create_rows (
    p_rows_tab IN t_rows_tab )
  IS
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
    INSERT INTO T_REPORTES (
      ID_REPORTE /*PK*/ /*FK*/,
      TIPO,
      CANTIDAD_EJECUCIONES,
      FECHA_ULTIMA_EJECUCION,
      CONSULTA_SQL,
      SQL_ULTIMA_EJECUCION,
      USUARIO_INSERCION,
      FECHA_INSERCION )
    VALUES (
      p_rows_tab(i).ID_REPORTE,
      p_rows_tab(i).TIPO,
      p_rows_tab(i).CANTIDAD_EJECUCIONES,
      p_rows_tab(i).FECHA_ULTIMA_EJECUCION,
      p_rows_tab(i).CONSULTA_SQL,
      p_rows_tab(i).SQL_ULTIMA_EJECUCION,
      substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      systimestamp );
  END create_rows;

  FUNCTION read_row (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES%ROWTYPE
  IS
    v_row T_REPORTES%ROWTYPE;
    CURSOR cur_row IS
      SELECT * FROM T_REPORTES
      WHERE
        ID_REPORTE = p_id_reporte;
  BEGIN
    OPEN cur_row;
    FETCH cur_row INTO v_row;
    CLOSE cur_row;
    RETURN v_row;
  END read_row;

  PROCEDURE read_row (
    p_id_reporte             IN            T_REPORTES.ID_REPORTE%TYPE /*PK*/ /*FK*/,
    p_tipo                      OUT NOCOPY T_REPORTES.TIPO%TYPE,
    p_cantidad_ejecuciones      OUT NOCOPY T_REPORTES.CANTIDAD_EJECUCIONES%TYPE,
    p_fecha_ultima_ejecucion    OUT NOCOPY T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE,
    p_consulta_sql              OUT NOCOPY T_REPORTES.CONSULTA_SQL%TYPE,
    p_sql_ultima_ejecucion      OUT NOCOPY T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE,
    p_usuario_insercion         OUT NOCOPY T_REPORTES.USUARIO_INSERCION%TYPE,
    p_fecha_insercion           OUT NOCOPY T_REPORTES.FECHA_INSERCION%TYPE,
    p_usuario_modificacion      OUT NOCOPY T_REPORTES.USUARIO_MODIFICACION%TYPE,
    p_fecha_modificacion        OUT NOCOPY T_REPORTES.FECHA_MODIFICACION%TYPE )
  IS
    v_row T_REPORTES%ROWTYPE;
  BEGIN
    v_row := read_row (
      p_id_reporte => p_id_reporte );
    p_tipo                   := v_row.TIPO; 
    p_cantidad_ejecuciones   := v_row.CANTIDAD_EJECUCIONES; 
    p_fecha_ultima_ejecucion := v_row.FECHA_ULTIMA_EJECUCION; 
    p_consulta_sql           := v_row.CONSULTA_SQL; 
    p_sql_ultima_ejecucion   := v_row.SQL_ULTIMA_EJECUCION; 
    p_usuario_insercion      := v_row.USUARIO_INSERCION; 
    p_fecha_insercion        := v_row.FECHA_INSERCION; 
    p_usuario_modificacion   := v_row.USUARIO_MODIFICACION; 
    p_fecha_modificacion     := v_row.FECHA_MODIFICACION; 
  END read_row;

  FUNCTION read_rows (
    p_ref_cursor IN t_strong_ref_cursor )
  RETURN t_rows_tab
  IS
    v_return t_rows_tab;
  BEGIN
    IF (p_ref_cursor%ISOPEN) THEN
      g_bulk_completed := FALSE;
      FETCH p_ref_cursor BULK COLLECT INTO v_return LIMIT g_bulk_limit;
      IF (v_return.COUNT < g_bulk_limit) THEN
        g_bulk_completed := TRUE;
      END IF;
    END IF;
    RETURN v_return;
  END read_rows;

  FUNCTION update_row (
    p_id_reporte             IN T_REPORTES.ID_REPORTE%TYPE DEFAULT NULL /*PK*/ /*FK*/,
    p_tipo                   IN T_REPORTES.TIPO%TYPE,
    p_cantidad_ejecuciones   IN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE,
    p_fecha_ultima_ejecucion IN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE,
    p_consulta_sql           IN T_REPORTES.CONSULTA_SQL%TYPE,
    p_sql_ultima_ejecucion   IN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE )
  RETURN T_REPORTES.ID_REPORTE%TYPE
  IS
    v_return T_REPORTES.ID_REPORTE%TYPE; 
  BEGIN
    UPDATE
      T_REPORTES
    SET
      TIPO                   = p_tipo,
      CANTIDAD_EJECUCIONES   = p_cantidad_ejecuciones,
      FECHA_ULTIMA_EJECUCION = p_fecha_ultima_ejecucion,
      CONSULTA_SQL           = p_consulta_sql,
      SQL_ULTIMA_EJECUCION   = p_sql_ultima_ejecucion,
      USUARIO_MODIFICACION   = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION     = systimestamp
    WHERE
      ID_REPORTE = p_id_reporte
    RETURN 
      ID_REPORTE
    INTO
      v_return;
    RETURN v_return;
  END update_row;

  PROCEDURE update_row (
    p_id_reporte             IN T_REPORTES.ID_REPORTE%TYPE DEFAULT NULL /*PK*/ /*FK*/,
    p_tipo                   IN T_REPORTES.TIPO%TYPE,
    p_cantidad_ejecuciones   IN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE,
    p_fecha_ultima_ejecucion IN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE,
    p_consulta_sql           IN T_REPORTES.CONSULTA_SQL%TYPE,
    p_sql_ultima_ejecucion   IN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE )
  IS
  BEGIN
    UPDATE
      T_REPORTES
    SET
      TIPO                   = p_tipo,
      CANTIDAD_EJECUCIONES   = p_cantidad_ejecuciones,
      FECHA_ULTIMA_EJECUCION = p_fecha_ultima_ejecucion,
      CONSULTA_SQL           = p_consulta_sql,
      SQL_ULTIMA_EJECUCION   = p_sql_ultima_ejecucion,
      USUARIO_MODIFICACION   = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION     = systimestamp
    WHERE
      ID_REPORTE = p_id_reporte;
  END update_row;

  FUNCTION update_row (
    p_row IN T_REPORTES%ROWTYPE )
  RETURN T_REPORTES.ID_REPORTE%TYPE
  IS
  BEGIN
    RETURN update_row (
      p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
      p_tipo                   => p_row.TIPO,
      p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
      p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
      p_consulta_sql           => p_row.CONSULTA_SQL,
      p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
  END update_row;

  PROCEDURE update_row (
    p_row IN T_REPORTES%ROWTYPE )
  IS
  BEGIN
    update_row (
      p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
      p_tipo                   => p_row.TIPO,
      p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
      p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
      p_consulta_sql           => p_row.CONSULTA_SQL,
      p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
  END update_row;

  PROCEDURE update_rows (
    p_rows_tab IN t_rows_tab )
  IS
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
      UPDATE
        T_REPORTES
      SET
        TIPO                   = p_rows_tab(i).TIPO,
        CANTIDAD_EJECUCIONES   = p_rows_tab(i).CANTIDAD_EJECUCIONES,
        FECHA_ULTIMA_EJECUCION = p_rows_tab(i).FECHA_ULTIMA_EJECUCION,
        CONSULTA_SQL           = p_rows_tab(i).CONSULTA_SQL,
        SQL_ULTIMA_EJECUCION   = p_rows_tab(i).SQL_ULTIMA_EJECUCION,
        USUARIO_MODIFICACION   = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
        FECHA_MODIFICACION     = systimestamp
      WHERE
        ID_REPORTE = p_rows_tab(i).ID_REPORTE;
  END update_rows;

  PROCEDURE delete_row (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  IS
  BEGIN
    DELETE FROM T_REPORTES
    WHERE
      ID_REPORTE = p_id_reporte;
  END delete_row;

  PROCEDURE delete_rows (
    p_rows_tab IN t_rows_tab )
  IS
  BEGIN
    FORALL i IN INDICES OF p_rows_tab
      DELETE FROM T_REPORTES
       WHERE ID_REPORTE = p_rows_tab(i).ID_REPORTE;
  END delete_rows;

  FUNCTION create_or_update_row (
    p_id_reporte             IN T_REPORTES.ID_REPORTE%TYPE DEFAULT NULL /*PK*/ /*FK*/,
    p_tipo                   IN T_REPORTES.TIPO%TYPE,
    p_cantidad_ejecuciones   IN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE,
    p_fecha_ultima_ejecucion IN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE,
    p_consulta_sql           IN T_REPORTES.CONSULTA_SQL%TYPE,
    p_sql_ultima_ejecucion   IN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE )
  RETURN T_REPORTES.ID_REPORTE%TYPE
  IS
  BEGIN
    IF row_exists(
      p_id_reporte => p_id_reporte
    )
    THEN
      RETURN update_row (
        p_id_reporte             => p_id_reporte /*PK*/ /*FK*/,
        p_tipo                   => p_tipo,
        p_cantidad_ejecuciones   => p_cantidad_ejecuciones,
        p_fecha_ultima_ejecucion => p_fecha_ultima_ejecucion,
        p_consulta_sql           => p_consulta_sql,
        p_sql_ultima_ejecucion   => p_sql_ultima_ejecucion );
    ELSE
      RETURN create_row (
        p_id_reporte             => p_id_reporte /*PK*/ /*FK*/,
        p_tipo                   => p_tipo,
        p_cantidad_ejecuciones   => p_cantidad_ejecuciones,
        p_fecha_ultima_ejecucion => p_fecha_ultima_ejecucion,
        p_consulta_sql           => p_consulta_sql,
        p_sql_ultima_ejecucion   => p_sql_ultima_ejecucion );
    END IF;
  END create_or_update_row;

  PROCEDURE create_or_update_row (
    p_id_reporte             IN T_REPORTES.ID_REPORTE%TYPE DEFAULT NULL /*PK*/ /*FK*/,
    p_tipo                   IN T_REPORTES.TIPO%TYPE,
    p_cantidad_ejecuciones   IN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE,
    p_fecha_ultima_ejecucion IN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE,
    p_consulta_sql           IN T_REPORTES.CONSULTA_SQL%TYPE,
    p_sql_ultima_ejecucion   IN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE )
  IS
  BEGIN
    IF row_exists(
      p_id_reporte => p_id_reporte
    )
    THEN
      update_row (
        p_id_reporte             => p_id_reporte /*PK*/ /*FK*/,
        p_tipo                   => p_tipo,
        p_cantidad_ejecuciones   => p_cantidad_ejecuciones,
        p_fecha_ultima_ejecucion => p_fecha_ultima_ejecucion,
        p_consulta_sql           => p_consulta_sql,
        p_sql_ultima_ejecucion   => p_sql_ultima_ejecucion );
    ELSE
      create_row (
        p_id_reporte             => p_id_reporte /*PK*/ /*FK*/,
        p_tipo                   => p_tipo,
        p_cantidad_ejecuciones   => p_cantidad_ejecuciones,
        p_fecha_ultima_ejecucion => p_fecha_ultima_ejecucion,
        p_consulta_sql           => p_consulta_sql,
        p_sql_ultima_ejecucion   => p_sql_ultima_ejecucion );
    END IF;
  END create_or_update_row;

  FUNCTION create_or_update_row (
    p_row IN T_REPORTES%ROWTYPE )
  RETURN T_REPORTES.ID_REPORTE%TYPE
  IS
  BEGIN
    IF row_exists(
      p_id_reporte => p_row.ID_REPORTE
    )
    THEN
      RETURN update_row (
        p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
        p_tipo                   => p_row.TIPO,
        p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
        p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
        p_consulta_sql           => p_row.CONSULTA_SQL,
        p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
    ELSE
      RETURN create_row (
        p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
        p_tipo                   => p_row.TIPO,
        p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
        p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
        p_consulta_sql           => p_row.CONSULTA_SQL,
        p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
    END IF;
  END create_or_update_row;

  PROCEDURE create_or_update_row (
    p_row IN T_REPORTES%ROWTYPE )
  IS
  BEGIN
    IF row_exists(
      p_id_reporte => p_row.ID_REPORTE
    )
    THEN
      update_row (
        p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
        p_tipo                   => p_row.TIPO,
        p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
        p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
        p_consulta_sql           => p_row.CONSULTA_SQL,
        p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
    ELSE
      create_row (
        p_id_reporte             => p_row.ID_REPORTE /*PK*/ /*FK*/,
        p_tipo                   => p_row.TIPO,
        p_cantidad_ejecuciones   => p_row.CANTIDAD_EJECUCIONES,
        p_fecha_ultima_ejecucion => p_row.FECHA_ULTIMA_EJECUCION,
        p_consulta_sql           => p_row.CONSULTA_SQL,
        p_sql_ultima_ejecucion   => p_row.SQL_ULTIMA_EJECUCION );
    END IF;
  END create_or_update_row;

  FUNCTION get_tipo (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.TIPO%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).TIPO;
  END get_tipo;

  FUNCTION get_cantidad_ejecuciones (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).CANTIDAD_EJECUCIONES;
  END get_cantidad_ejecuciones;

  FUNCTION get_fecha_ultima_ejecucion (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).FECHA_ULTIMA_EJECUCION;
  END get_fecha_ultima_ejecucion;

  FUNCTION get_consulta_sql (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.CONSULTA_SQL%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).CONSULTA_SQL;
  END get_consulta_sql;

  FUNCTION get_sql_ultima_ejecucion (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).SQL_ULTIMA_EJECUCION;
  END get_sql_ultima_ejecucion;

  FUNCTION get_usuario_insercion (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.USUARIO_INSERCION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).USUARIO_INSERCION;
  END get_usuario_insercion;

  FUNCTION get_fecha_insercion (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.FECHA_INSERCION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).FECHA_INSERCION;
  END get_fecha_insercion;

  FUNCTION get_usuario_modificacion (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.USUARIO_MODIFICACION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).USUARIO_MODIFICACION;
  END get_usuario_modificacion;

  FUNCTION get_fecha_modificacion (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/ )
  RETURN T_REPORTES.FECHA_MODIFICACION%TYPE
  IS
  BEGIN
    RETURN read_row (
      p_id_reporte => p_id_reporte ).FECHA_MODIFICACION;
  END get_fecha_modificacion;

  PROCEDURE set_tipo (
    p_id_reporte IN T_REPORTES.ID_REPORTE%TYPE /*PK*/,
    p_tipo       IN T_REPORTES.TIPO%TYPE )
  IS
  BEGIN
    UPDATE
      T_REPORTES
    SET
      TIPO                 = p_tipo,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      ID_REPORTE = p_id_reporte;
  END set_tipo;

  PROCEDURE set_cantidad_ejecuciones (
    p_id_reporte           IN T_REPORTES.ID_REPORTE%TYPE /*PK*/,
    p_cantidad_ejecuciones IN T_REPORTES.CANTIDAD_EJECUCIONES%TYPE )
  IS
  BEGIN
    UPDATE
      T_REPORTES
    SET
      CANTIDAD_EJECUCIONES = p_cantidad_ejecuciones,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      ID_REPORTE = p_id_reporte;
  END set_cantidad_ejecuciones;

  PROCEDURE set_fecha_ultima_ejecucion (
    p_id_reporte             IN T_REPORTES.ID_REPORTE%TYPE /*PK*/,
    p_fecha_ultima_ejecucion IN T_REPORTES.FECHA_ULTIMA_EJECUCION%TYPE )
  IS
  BEGIN
    UPDATE
      T_REPORTES
    SET
      FECHA_ULTIMA_EJECUCION = p_fecha_ultima_ejecucion,
      USUARIO_MODIFICACION   = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION     = systimestamp
    WHERE
      ID_REPORTE = p_id_reporte;
  END set_fecha_ultima_ejecucion;

  PROCEDURE set_consulta_sql (
    p_id_reporte   IN T_REPORTES.ID_REPORTE%TYPE /*PK*/,
    p_consulta_sql IN T_REPORTES.CONSULTA_SQL%TYPE )
  IS
  BEGIN
    UPDATE
      T_REPORTES
    SET
      CONSULTA_SQL         = p_consulta_sql,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      ID_REPORTE = p_id_reporte;
  END set_consulta_sql;

  PROCEDURE set_sql_ultima_ejecucion (
    p_id_reporte           IN T_REPORTES.ID_REPORTE%TYPE /*PK*/,
    p_sql_ultima_ejecucion IN T_REPORTES.SQL_ULTIMA_EJECUCION%TYPE )
  IS
  BEGIN
    UPDATE
      T_REPORTES
    SET
      SQL_ULTIMA_EJECUCION = p_sql_ultima_ejecucion,
      USUARIO_MODIFICACION = substr(coalesce(k_sistema.f_usuario, user), 1, 300),
      FECHA_MODIFICACION   = systimestamp
    WHERE
      ID_REPORTE = p_id_reporte;
  END set_sql_ultima_ejecucion;

  FUNCTION get_default_row
  RETURN T_REPORTES%ROWTYPE
  IS
    v_row T_REPORTES%ROWTYPE;
  BEGIN
    v_row.USUARIO_INSERCION := SUBSTR(USER, 1, 300) ;
    v_row.FECHA_INSERCION   := SYSTIMESTAMP ;
    RETURN v_row;
  END get_default_row;

END T_REPORTES_API;
/

