/* ==================== T_SIGNIFICADO_DOMINIOS ==================== */
set define off
declare
  type   t_clob is table of clob index by binary_integer;
  l_clob t_clob;
  type   t_varchar2 is table of varchar2(64) index by binary_integer;
  l_varchar2 t_varchar2;
begin

  null;
  -- start generation of records
  -----------------------------------

  l_clob(1) :=q'!EXTENSION_REPORTE!';
  l_clob(2) :=q'!Extensiones permitidas para archivos de reportes!';
  l_clob(3) :=q'!El valor de la referencia indica el tipo MIME relacionado a la extensión!';
  l_clob(4) :=q'!S!';
  l_clob(5) :=q'!GEN!';

  insert into t_significado_dominios_dml_v
  (
     "DOMINIO"
    ,"DESCRIPCION"
    ,"DETALLE"
    ,"ACTIVO"
    ,"ID_DOMINIO"
  )
  values
  (
     to_char(l_clob(1))
    ,to_char(l_clob(2))
    ,to_char(l_clob(3))
    ,to_char(l_clob(4))
    ,to_char(l_clob(5))
  );

end;
/
/* ==================== T_SIGNIFICADOS ==================== */
set define off
declare
  type   t_clob is table of clob index by binary_integer;
  l_clob t_clob;
  type   t_varchar2 is table of varchar2(64) index by binary_integer;
  l_varchar2 t_varchar2;
begin

  null;
  -- start generation of records
  -----------------------------------

  l_clob(1) :=q'!EXTENSION_REPORTE!';
  l_clob(2) :=q'!CSV!';
  l_clob(3) :=q'!REPORTE CSV!';
  l_clob(4) :=q'!text/csv!';
  l_clob(5) :=q'!S!';
  l_clob(6) :=q'!!';

  insert into t_significados_dml_v
  (
     "DOMINIO"
    ,"CODIGO"
    ,"SIGNIFICADO"
    ,"REFERENCIA"
    ,"ACTIVO"
    ,"REFERENCIA_2"
  )
  values
  (
     to_char(l_clob(1))
    ,to_char(l_clob(2))
    ,to_char(l_clob(3))
    ,to_char(l_clob(4))
    ,to_char(l_clob(5))
    ,to_char(l_clob(6))
  );

  l_clob(1) :=q'!EXTENSION_REPORTE!';
  l_clob(2) :=q'!DOCX!';
  l_clob(3) :=q'!REPORTE DOCX!';
  l_clob(4) :=q'!application/octet-stream!';
  l_clob(5) :=q'!S!';
  l_clob(6) :=q'!!';

  insert into t_significados_dml_v
  (
     "DOMINIO"
    ,"CODIGO"
    ,"SIGNIFICADO"
    ,"REFERENCIA"
    ,"ACTIVO"
    ,"REFERENCIA_2"
  )
  values
  (
     to_char(l_clob(1))
    ,to_char(l_clob(2))
    ,to_char(l_clob(3))
    ,to_char(l_clob(4))
    ,to_char(l_clob(5))
    ,to_char(l_clob(6))
  );

  l_clob(1) :=q'!EXTENSION_REPORTE!';
  l_clob(2) :=q'!HTML!';
  l_clob(3) :=q'!REPORTE HTML!';
  l_clob(4) :=q'!text/html!';
  l_clob(5) :=q'!S!';
  l_clob(6) :=q'!!';

  insert into t_significados_dml_v
  (
     "DOMINIO"
    ,"CODIGO"
    ,"SIGNIFICADO"
    ,"REFERENCIA"
    ,"ACTIVO"
    ,"REFERENCIA_2"
  )
  values
  (
     to_char(l_clob(1))
    ,to_char(l_clob(2))
    ,to_char(l_clob(3))
    ,to_char(l_clob(4))
    ,to_char(l_clob(5))
    ,to_char(l_clob(6))
  );

  l_clob(1) :=q'!EXTENSION_REPORTE!';
  l_clob(2) :=q'!PDF!';
  l_clob(3) :=q'!REPORTE PDF!';
  l_clob(4) :=q'!application/pdf!';
  l_clob(5) :=q'!S!';
  l_clob(6) :=q'!!';

  insert into t_significados_dml_v
  (
     "DOMINIO"
    ,"CODIGO"
    ,"SIGNIFICADO"
    ,"REFERENCIA"
    ,"ACTIVO"
    ,"REFERENCIA_2"
  )
  values
  (
     to_char(l_clob(1))
    ,to_char(l_clob(2))
    ,to_char(l_clob(3))
    ,to_char(l_clob(4))
    ,to_char(l_clob(5))
    ,to_char(l_clob(6))
  );

  l_clob(1) :=q'!EXTENSION_REPORTE!';
  l_clob(2) :=q'!XLSX!';
  l_clob(3) :=q'!REPORTE XLSX!';
  l_clob(4) :=q'!application/octet-stream!';
  l_clob(5) :=q'!S!';
  l_clob(6) :=q'!!';

  insert into t_significados_dml_v
  (
     "DOMINIO"
    ,"CODIGO"
    ,"SIGNIFICADO"
    ,"REFERENCIA"
    ,"ACTIVO"
    ,"REFERENCIA_2"
  )
  values
  (
     to_char(l_clob(1))
    ,to_char(l_clob(2))
    ,to_char(l_clob(3))
    ,to_char(l_clob(4))
    ,to_char(l_clob(5))
    ,to_char(l_clob(6))
  );

end;
/
